...
namespace alib
{
	namespace Math
	{
		...
		struct alignas(alignment) Vector4
		{
				...
				struct
				{
					...
					union
					{
						...
						struct
						{
							...
							strong_inline Vector4& operator = (Type aDesiredLengthSquared)
							{
								Vector4& vector = Memory::Cast<Vector4>(this, (-((int)sizeof(Type) * 3)));
						...
						struct
						{
							...
							strong_inline Type operator()(Vector4& second_vector) const
							{
								Vector4& vector = Memory::Cast<Vector4>(this, (-((int)sizeof(Type) * 3)));
							...
							strong_inline Vector4& operator = (Type aDesiredDotProduct)
							{
								Vector4& vector = Memory::Cast<Vector4>(this, (-((int)sizeof(Type) * 3)));
								Type currentLength = vector.Length();
						...
						struct
						{
							...
							strong_inline Vector4& operator = (Type aDesiredLength)
							{
								Vector4& vector = Memory::Cast<Vector4>(this, (-((int)sizeof(Type) * 3)));
						...
						struct
						{
							strong_inline operator Vector4() const
							{
								Vector4& vector = Memory::Cast<Vector4>(this, (-((int)sizeof(Type) * 3)));
							...
							strong_inline Vector4 operator()() const
							{
								Vector4 result = (*this);
								return result;
							}
						...
						struct
						{
							strong_inline operator Vector4&()
							{
								Vector4& vector = Memory::Cast<Vector4>(this, (-((int)sizeof(Type) * 3)));
								Type length = vector.length;
								if (length > 0.0f)
							...
							strong_inline Vector4& operator()() 
							{
								Vector4& result = (*this);
								return result;
							}
			...
			strong_inline Vector4& operator = (const Vector4 & aAssignment)
			{
				x = aAssignment.x;
				y = aAssignment.y;
				z = aAssignment.z;
			...
			template<typename Type, int Offset1, int Offset2, int Offset3, int Offset4>
			strong_inline Vector4& operator = (const Vector4Swizzle<Type, Offset1, Offset2, Offset3, Offset4> & aAssignment)
			{
				x = aAssignment.x;
				y = aAssignment.y;
				z = aAssignment.z;
			...
			strong_inline Vector4& operator = (const Type aAssignment)
			{
				x = aAssignment.x;
				y = aAssignment.y;
				z = aAssignment.z;
			...
			strong_inline Vector4& operator = (const float* aAssignment)
			{
				x = aAssignment[0];
				y = aAssignment[1];
				z = aAssignment[2];
			...
			//strong_inline Vector4(Vector4& aCopy)
			//{
			//	new (&x) Type(aCopy.x);
			//	new (&y) Type(aCopy.y);
			//	new (&z) Type(aCopy.z);
			...
			strong_inline Vector4(const Type & a_x, const Type & a_y, const Type & a_z, const Type & a_w) : data{ a_x, a_y, a_z, a_w }
			{

			}

			strong_inline Vector4(const Type & a_v) : data{ a_v, a_v, a_v, a_v }
			{

			}

			strong_inline Vector4() : data{ DefaultInitializationValue<Type>(), DefaultInitializationValue<Type>(), DefaultInitializationValue<Type>() , DefaultInitializationValue<Type>() }
			{
